...
namespace PBaszak\UltraMapper\Mapper\Domain\Modules\Checker\Strategy;
...
use PBaszak\UltraMapper\Mapper\Domain\Modules\Checker\Contract\CheckerStrategyInterface;
use PBaszak\UltraMapper\Mapper\Domain\Modules\Checker\Exception\CheckerException;

class RecursiveLoopChecker implements CheckerStrategyInterface
{
            ...
            foreach ($types as $type) {
                if ($actualCheckedClass == $type) {
                    ...
                    if (
                        ...
                        throw new CheckerException(sprintf('The %s::%s property is the recursive loop source.', $property->class->name, $property->originName), sprintf('There is a few ideas You can did right now. Use one of the following attributes for this property: Ignore, Groups with groups out of the current context (%s), MaxDepth.', implode(', ', $context->groups)));
                    }
                }
